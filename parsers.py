...
# Fixed HSBC pattern: DATE (01 Sep 25), DESCRIPTION, AMOUNT, optional BALANCE
LINE_RE = re.compile(
...
def parse_hsbc_pdf_bytes(data: bytes) -> pd.DataFrame:
    ...
    with pdfplumber.open(io.BytesIO(data)) as pdf:
        for page in pdf.pages:
            ...
            # Keep raw lines to preserve structure
    ...
    # HSBC sample uses 2-digit year like '25', we parse with exact format
    df["date"] = pd.to_datetime(df["date_raw"], format="%d %b %y", errors="coerce")
    # fallback: try 4-digit year if needed
    bad = df["date"].isna()
    if bad.any():
...
def categorize(description: str, amount: float) -> str:
    ...
    inbound = amount > 0
    # Inbound heuristics (eBay payouts / transfers)
